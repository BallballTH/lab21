...
Equipment::Equipment(int hp,int at,int de){
	hpmax = hp; atk = at; def = de;
}
...
void Unit::equip(Equipment *neweq){
	...
	if (equipment != NULL) {
        vector<int> oldSt = equipment->getStat();
        hpmax -= oldSt[0];
        atk -= oldSt[1];
        def -= oldSt[2];
    }
	
	vector<int> newSt = neweq->getStat();
	hpmax += newSt[0];
	atk += newSt[1];
	def += newSt[2];
